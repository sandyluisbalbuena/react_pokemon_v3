		...
		.then(response => {
			...
			response.data.cards.forEach(function(pokecard){
				...
				if(pokecard.rarity != undefined){
					...
					let $newItems = $(`<div id="`+pokecard.id+`" class="grid-item card-img-forSearch `+pokecard.rarity.toUpperCase().trim().replace(/\s/g, '').replace(/[\s~`!@#$%^&*()\-_=+[\]{}|;:'",.<>/?\\]/g, '')+`"><img src="`+pokecard.imageUrlHiRes+`" width="100%"/></div>`);
