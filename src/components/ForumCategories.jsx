		...
		<div className="card my-1 px-1 animate__animated animate__fadeIn animate__delay-1s" style={{ borderRadius: '5px', height: '100%' }} id="secondCard">
			<div className="card-body container-fluid">
				...
				{categories.map((category) => (
					<a onClick={()=>wew(category.name)} key={category.id} href={'#'+category.name}><li className="px-2 py-1 rounded list-group-item threads-latest my-2" style={{fontSize:'12px', textDecoration:'none', color:'black'}}>{category.name.toUpperCase()}</li></a>
				))}
				</ul>
			</div>
		</div>
